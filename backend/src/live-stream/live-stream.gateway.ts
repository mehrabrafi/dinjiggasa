...
export class LiveStreamGateway implements OnGatewayDisconnect {
  ...
  startStream(
    ...
    const ffmpegArgs = [
      '-i',
      '-', // Read from stdin (WebM chunks)
      '-c:v',
      'libx264', // Video codec
      '-preset',
      'veryfast', // Better quality than ultrafast, still fast enough for live
      '-tune',
      'zerolatency', // Minimal latency
      '-crf',
      '23', // Quality target (lower = better, 23 is default/good)
      '-maxrate',
      '3500k', // Max bitrate for 720p
      '-bufsize',
      '7000k', // Buffer size = 2x maxrate
      '-pix_fmt',
      'yuv420p',
      '-g',
      ...
      '-f',
      'flv', // Output format RTMP expects flv
      rtmpUrl,
    ];
    ...
    ffmpegProcess.on('close', (code, signal) => {
      console.log(
        `FFmpeg process for ${client.id} closed with code ${code} and signal ${signal}`,
      );
